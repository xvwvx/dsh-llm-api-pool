window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS =
      ...
      ".lp-select{background:var(--dsw-alias-bg-base);border:1px solid var(--dsw-alias-border-l1);color:var(--dsw-alias-label-primary);border-radius:6px;padding:4px 8px;font-size:12px}" +
      ...
      ".lp-limit{display:grid;grid-template-columns:70px 1fr auto;align-items:center;gap:8px;font-size:12px}" +
      ".lp-limit .lp-mono{white-space:nowrap}" +
      ".lp-bar{height:8px;min-width:56px;border-radius:99px;background:var(--dsw-alias-bg-layer-2);overflow:hidden;border:1px solid var(--dsw-alias-border-l1)}" +
    ...
    /**
     * Render an instant as a local date + time. Reset moments are future ISO
     * timestamps, so a bare clock time ("重置 08:00:00") is ambiguous as soon as
     * the instant lands on another day — the date is always shown, with the year
     * added when it is not the current year. Absent values render as "" and
     * unusable ones verbatim.
     */
    ...
    function PoolSettings() {
      ...
      const markRemote = (entryId, res) => {
        setRemoteMap((m) => Object.assign({}, m, { [entryId]: { windows: res.windows || null, source: res.source || null, fetchedAt: new Date().toISOString() } }));
      };
      ...
      const balanceEntry = (entry) => run("余额", callHost("balance", { id: entry.id }).then((r) => {
        if (r && r.ok && r.result) {
          ...
          return { msg: "官方端点不可达,回退本地估算: " + String(res.error || "unknown") };
        }
        return r;
      }));
      ...
      const moveEntry = (entry, delta) => run("排序", callHost("reorder", { id: entry.id, delta }));
      const setRouting = (mode) => run("切换顺序", callHost("config", { routing: mode }).then((r) => ({
        msg: "请求顺序已切换为" + ((r && r.routing) === "listed" ? "卡片顺序(从上到下依次尝试)" : "用量最少优先")
      })));
      ...
      const routing = (pool && pool.routing) || "score";
      const elems = [
        react.createElement("div", { key: "head", className: "lp-head" },
          ...
          react.createElement("div", { className: "lp-row" },
            react.createElement("span", { className: "lp-muted" }, "请求顺序"),
            react.createElement("select", {
              className: "lp-select",
              value: routing,
              disabled: !!busy,
              title: "按用量最少优先 = 自动挑累计用量/限额压力最小的订阅;按卡片顺序 = 从上到下依次尝试",
              onChange: (e) => setRouting(e.target.value)
            },
              react.createElement("option", { value: "score" }, "按用量最少优先"),
              react.createElement("option", { value: "listed" }, "按卡片顺序")
            ),
      ...
      const anyOfficial = entries.some((e) => remoteMap[e.id] || (e.usageRemote && e.usageRemote.windows));
      if (!anyOfficial && !busy) {
        elems.push(react.createElement("div", { key: "hint", className: "lp-hint" }, "提示:点击卡片上的「刷新限额 / 查询余额」会调用官方 /usage 端点(纯 API Key),百分比与美元即服务端实时数值;未点击、或官方端点不可达时显示本地估算。"));
      }

      if (routing === "listed" && entries.length > 1) {
        elems.push(react.createElement("div", { key: "order-hint", className: "lp-muted" }, "当前按卡片顺序请求:从上到下依次尝试,冷却中或已用满的条目自动沉底;用卡片上的 ↑ / ↓ 调整顺序。"));
      }
      ...
      for (let entryIndex = 0; entryIndex < entries.length; entryIndex += 1) {
        ...
        // Official windows come from this session's click first, then from the host's
        // persisted usageRemote cache: a page reload must not drop the card back to the
        // local estimate while official data is still on disk.
        const cached = entry.usageRemote && entry.usageRemote.windows ? entry.usageRemote : null;
        const remote = remoteMap[entry.id] || (cached ? { windows: cached.windows, source: null, fetchedAt: cached.fetchedAt || null } : null);
        ...
        if (limits && (limits.h5 || limits.week || limits.month)) {
          ...
          limElems.push(react.createElement("div", { key: "lim", className: "lp-limits" }, rows.map((r) => {
            ...
            const reset = official && st.resetsAt ? " · 重置 " + fmtDateTime(st.resetsAt) : "";
          ...
          if (official) {
            limElems.push(react.createElement("div", { key: "src", className: "lp-muted" }, (remote.source ? "来源 " + remote.source : "来源 官方缓存") + " · 查询于 " + fmtDateTime(remote.fetchedAt)));
          }
        } else {
        ...
        elems.push(react.createElement("div", { key: entry.id, className: "lp-card" },
          react.createElement("div", { className: "lp-row" },
            react.createElement("span", { className: "lp-muted" }, "#" + (entryIndex + 1)),
          ...
          react.createElement("div", { className: "lp-row" },
            react.createElement("button", { className: "lp-btn", disabled: !!busy || entryIndex === 0, title: "上移(「按卡片顺序」时决定请求优先级)", onClick: () => moveEntry(entry, -1) }, "↑"),
            react.createElement("button", { className: "lp-btn", disabled: !!busy || entryIndex === entries.length - 1, title: "下移(「按卡片顺序」时决定请求优先级)", onClick: () => moveEntry(entry, 1) }, "↓"),
