...
async function apply(ctx) {
  ...
  const poolAdapter = {
    ...
    async resolveModel(provider, model) {
      return { provider: String(provider), id: String(model), name: String(model) };
    },
    async *stream(options) {
      const chatArgs = {
        temperature: options.temperature,
        max_tokens: options.maxTokens,
        ...(options.tools && options.tools.length ? { extra: { tools: options.tools.map((t) => ({ type: 'function', function: t })) } } : {}),
      };
